...
def parse_arguments():
    ...
    parser.add_argument('-d', '--dither', required=False, metavar='dither', type=str_as_bool, nargs='?', default=True, help='Allow dithering. Defaults to True.')
    parser.add_argument('-a', '--alpha', required=False, metavar='alpha', type=float, default=.6, help='Threshold for visibility for images with alpha channel. Defaults to .6.')
...
if __name__ == "__main__":
    ...
    p = Pyxelate(1, 1, color=args.colors, dither=args.dither, alpha=args.alpha, regenerate_palette=args.regenerate_palette, random_state=args.random_state)
